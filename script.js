...
function atualizarLista() {
  ...
  clientes.forEach(cliente => {
    if ((fs && cliente.status !== fs) ||
        (ft && cliente.tipo !== ft) ||
        (fp && cliente.prioridade !== fp) ||
        (textoBusca && !cliente.nome.toLowerCase().includes(textoBusca) && !cliente.empresa.toLowerCase().includes(textoBusca))) return;

    total += cliente.valor || 0;

    const card = document.createElement('div');
    card.className = `bg-white shadow-xl rounded-xl p-5 flex flex-col gap-2 border-l-8 ${
      cliente.prioridade === 'alta' ? 'border-red-500' :
      cliente.prioridade === 'média' ? 'border-yellow-400' : 'border-blue-400'
    }`;

    card.innerHTML = `
      <div class="flex justify-between items-center">
        <h3 class="text-lg font-bold">${cliente.nome || 'Sem nome'}</h3>
      ...
      <div class="flex gap-2 text-sm">
        <span class="px-2 py-1 rounded-full text-white ${cliente.status === 'entregue' ? 'bg-green-500' : 'bg-blue-500'}">${cliente.status}</span>
        <span class="px-2 py-1 rounded-full bg-gray-200 text-gray-800">${cliente.prioridade}</span>
      </div>
  ...
  document.querySelectorAll('.btn-edit').forEach(btn => btn.addEventListener('click', () => editarCliente(btn.dataset.id)));
  document.querySelectorAll('.btn-delete').forEach(btn => btn.addEventListener('click', () => excluirCliente(btn.dataset.id)));
...
function excluirCliente(id) {
  if (confirm('Tem certeza que deseja excluir este cliente?')) {
    remove(ref(db, `clientes/${id}`)).catch(error => alert(`Erro ao excluir: ${error.message}`));
  }
}
  ...
  reader.onload = (event) => {
    try {
      ...
      if (confirm(`Importar ${dados.length} clientes?`)) {
        const batch = dados.map(({ id, ...cliente }) => push(clientesRef, cliente));
        Promise.all(batch)
